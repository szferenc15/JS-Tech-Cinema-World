...
import { Chair, BookingTicket } from './../../interfaces/booking-ticket.interface';
...
export class RoomStructureComponent implements OnInit {
	...
	constructor(private renderer: Renderer, private bookingService: BookingService) {}
	...
	ngOnInit() {
		...
		this.bookingService.getRoomDimension().subscribe((roomDimension: RoomDimension) => {
			...
			this.bookingService.getBookingsOnSelectedScreening().subscribe((bookings: BookingTicket[]) => {
				for (let j = 0; j < bookings.length; j++) {
					this.occupiedChairs.push({ row: bookings[j].row, chair: bookings[j].chair });
				}
			});
		});
	}
